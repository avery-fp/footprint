...
export default function TileImage({ src, alt, sizes, index, aspect, layout, size, isPublicView = false }: TileImageProps) {
  const displaySrc = isPublicView ? getPublicImageUrl(src) || src : src
  const [failed, setFailed] = useState(false)
  const [videoFailed, setVideoFailed] = useState(false)
  const [loaded, setLoaded] = useState(() => isPublicView && settledPublicMedia.has(displaySrc))
  ...
  useEffect(() => {
    ...
    setLoaded(true)
    if (isPublicView) settledPublicMedia.add(displaySrc)
    ...
    return (
      <div ref={containerRef} className="absolute inset-0">
        ...
        <img
          ...
          onLoad={() => {
            setLoaded(true)
            if (!settledPublicMedia.has(displaySrc)) {
              settledPublicMedia.add(displaySrc)
