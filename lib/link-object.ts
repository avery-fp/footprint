...
// Generic machine-generated labels that should never surface as tile headlines.
// Case-insensitive match applied after trimming and entity-decoding.
...
function isAutoGeneratedTitle(title: string, url: string): boolean {
  ...
  if (GENERIC_AUTO_TITLES.has(t)) return true
  // Domain-only: title exactly matches the URL's hostname (with or without www.)
  try {
...
export function sanitizeLinkMeta(
  ...
  const fallback = (provider !== domain ? provider : domain) || 'link'

  const rawTitle = raw.title ? decodeHtmlEntities(raw.title).trim() : ''
  const title = (!rawTitle || isAutoGeneratedTitle(rawTitle, url)) ? fallback : rawTitle

  const rawAuthor = raw.author ? decodeHtmlEntities(raw.author).trim() : ''
  const rawCreator = raw.creator ? decodeHtmlEntities(raw.creator).trim() : ''
  const creator = rawAuthor || rawCreator || null

  const rawDescription = raw.description ? decodeHtmlEntities(raw.description).trim() : ''
